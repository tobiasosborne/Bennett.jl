...
5. **LLVM IR IS NOT STABLE.** LLVM IR output is not a stable API. Never assume specific IR formatting, instruction ordering, or naming conventions. The LLVM.jl C API walker (`ir_extract.jl`) is the source of truth — not regex parsing. When LLVM changes its output, `ir_extract.jl` must adapt. Always use `optimize=false` for predictable IR when testing. **Bennett-t9rh:** `optimize=true` IR is produced under a PINNED TargetMachine (`x86-64-v3` on x86_64, `generic` elsewhere; O2 — `src/extract/target_pin.jl`), so `julia -C`/`-O` and the host CPU no longer change gate counts. Never study the host's `code_llvm(optimize=true)` output — it is not what Bennett compiles; use `Bennett.extract_ir(f, T)` or a `julia -C x86-64-v3` session.

6. **GATE COUNTS ARE REGRESSION BASELINES (per explicit strategy).** Verified gate counts (documented in `worklog/` chunks and pinned in `test/test_gate_count_regression.jl`) are regression tests. If a change alters gate counts for an explicit-strategy invocation, that is a signal — investigate whether it's an improvement or a bug. Key baselines for `add=:ripple, fold_constants=true`: i8 `x+1` = 58 gates, i16 = 114, i32 = 226, i64 = 450; each doubling obeys `total(2W) == 2·total(W) - 2`. Toffoli counts: 12/28/60/124 (each doubling obeys `T(2W) == 2·T(W) + 4`). See `test/test_gate_count_regression.jl` for the pinned assertions. **Bennett-hjwp / U150**: baselines are pinned to *explicit* strategy kwargs (e.g. `add=:ripple`), NOT to `:auto` defaults — this lets the default-strategy dispatcher evolve (e.g. `add=:auto` migrating from `:ripple` to `:qcla` once mature) without tripping the regression tests. If you change a default-strategy choice, document the new default-vs-explicit gate-count delta in BENCHMARKS.md but do NOT update the explicit-strategy baselines unless you intentionally changed that strategy's lowering.
...
13. **SOFT-FLOAT MUST BE BIT-EXACT (Float64 only).** The soft-float library (`src/softfloat/`) implements IEEE 754 operations in pure integer arithmetic. Every Float64 soft-float function must be bit-exact against Julia's native floating-point operations. Test with random inputs AND edge cases (0, -0, Inf, NaN, subnormals, overflow boundaries). **Float32 deviation (Bennett-3rph / U137):** there are no native f32 arithmetic primitives; f32 operations in mixed-precision IR are routed through `soft_fpext → f64-op → soft_fptrunc`, which double-rounds and is **NOT** bit-exact against hardware f32. `reversible_compile(f, Float32)` is rejected. Native 24-bit-mantissa f32 paths are future work — until then, the bit-exact contract applies only to f64. See `src/softfloat/fpconv.jl` docstring header for full details.
...
Bennett.jl/                         # Project root. PRDs and CLAUDE.md live alongside Project.toml.
  ...
  test/                             # 328 test_*.jl files, all registered in runtests.jl (a few behind BENNETT_T5/RESEARCH gates) / ~1.53M assertions (as of 2026-09-24). On a 16 GB box a single Pkg.test process can OOM (~12 GB RSS) — run chunked (see worklog/108)
