...
@testset "Bennett-kmuj / U106 — callee registration groups" begin
    ...
    @testset "total registered count matches expected" begin
        ...
        @test n_grouped == 103  # mq6f: +1 (soft_round_away); k2w6+p19b: +6 (_CALLEES_FP_MINMAX, new group); z2dj: +2 (_CALLEES_PERSISTENT, new group); 6883: +2 (okasaki_pmap_set/get); d746: +2 (hamt_pmap_set/get); qi6c: +2 (cf_pmap_set/get)

        # _known_callees may contain more if anything else (test fixtures,
        # other modules) registered, but it must contain at LEAST the 103
        # we register from the groups.
        @test length(Bennett._known_callees) >= 103
    end
    ...
    @testset "group sizes match the documented partition" begin
        ...
        @test length(Bennett._CALLEES_PERSISTENT)         == 8   # z2dj NEW group: linear_scan_pmap_set/get; 6883: +okasaki_pmap_set/get; d746: +hamt_pmap_set/get; qi6c: +cf_pmap_set/get
    end
end
