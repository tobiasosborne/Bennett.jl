; Bennett-lx5h: llvm.vector.reduce.fadd.v4f64.
; LLVM langref: returns `start + vec[0] + vec[1] + vec[2] + vec[3]` (strict
; left-to-right when no `reassoc` flag is set). The first arg is a scalar
; start value, the second is the vector. The .ll passes/returns f64 bit
; patterns as i64 to keep the simulate harness UInt64-typed.

declare double @llvm.vector.reduce.fadd.v4f64(double, <4 x double>)

define i64 @lx5h_reduce_fadd_v4f64(i64 %s, i64 %a, i64 %b, i64 %c, i64 %d) {
entry:
  ...
  %r  = call double @llvm.vector.reduce.fadd.v4f64(double %fs, <4 x double> %v3)
