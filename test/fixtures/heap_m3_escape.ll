; M3 reject fixture — element-buffer pointer escapes into a non-allowlisted
; callee. This is the otherwise-valid f_tj1 push×3 + reduce IR (the un-gapped
; sibling of heap_m3_gap.ll) with ONE added edit: a `call void @some_sink(ptr
; %memory_data)` just before the `ret`, plus the matching `declare`.
; `%memory_data` is a member of the `data_roots` buffer-base family, so the
; escape trips obligation M2-O5 — the `data_roots`-generalised no-escape check
; in `_prove_partition_sound` (run first by `_prove_growend_partition_sound`).
; M3 has no separate escape obligation: the data_roots refactor made M2-O5 the
; one general buffer-base escape check (see src/extract/heap.jl, the G1->G3
; label gap). Reject message: "M2-O5: an element-data / Memory / Array pointer
; escapes ... — the heap array is not isolable".
; ModuleID = 'f_tj1'
...
L181:                                             ; preds = %L68, %L44
  ...
  %memoryref_data59 = getelementptr i8, ptr %.pre-phi154, i64 2, !dbg !70
  ...
  call void @some_sink(ptr %memory_data), !dbg !104
