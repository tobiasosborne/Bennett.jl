...
@testset "Bennett-40ys — instance-less callee keys (closures + functors)" begin
    ...
    @testset "(I) push! reaches the NEXT named wall, not an UndefRefError" begin
        ...
        @test !occursin("_foz5_confined_dead_bounds", e.msg)
        @test !occursin("_57hd_value_identity_cluster", e.msg)
        ...
        @test occursin("_p06b_cell_ptr_target_kind", e.msg)   # names the predicate
        @test occursin("SILENTLY SKIPS", e.msg)
        ...
        @test !occursin("new::Array.ref.mem", e.msg)
        @test !occursin("Bennett-5viz", e.msg)       # 5viz must not be the new wall
