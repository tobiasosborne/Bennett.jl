...
# REPORT
Write to: `reviews/2026-09-26-astra/B-extract-core.md` (relative to the repo root, which is your working directory).

# SCOPE — B-extract-core
Repo: Bennett.jl. Your scope is IR EXTRACTION, core path: `src/ir_extract.jl` (include manifest) and under `src/extract/`: entry.jl, callees.jl, callgraph.jl, julia_set.jl, errors.jl, sret.jl, module_walk.jl, instructions.jl, heap.jl, constexpr.jl, vectors.jl, helpers.jl, target_pin.jl (new: pinned x86-64-v3 TargetMachine at O2, Bennett-t9rh). The mem=:vm recognisers (dict_vm.jl, vector_vm*.jl) are ANOTHER reviewer's scope; read them only if a finding needs it. Also `src/ir_types.jl`, `src/callees.jl`, `src/memssa.jl`, `src/ir_parser.jl` (legacy).
Highest-value targets:
1. Soundness of the IR → ParsedIR conversion: every LLVM opcode handled in `instructions.jl` — are flags (nsw/nuw, exact, poison), predicate mapping (`_PRED_MAP`), type widths (`_iwidth/_type_width`, i1 vs i8 for Bool, pointer widths), constant operands (ConstantExpr folding in constexpr.jl), GlobalAlias, undef/poison, vector scalarisation (`vectors.jl`; Bennett-t9rh added poison-lane propagation — audit it) all translated correctly? Extract real IR with `Bennett.extract_ir(f, (T,))`/`extract_parsed_ir` and compare against `code_llvm` under the pinned target. Hunt for silent acceptance: any instruction/intrinsic that is dropped, treated as a no-op, or mapped to a "close enough" opcode.
2. `heap.jl` (GC-preamble recogniser) and `sret.jl`: pattern-matching on Julia codegen shapes. Which shapes are assumed and what happens when Julia emits a variant (different field order, extra bitcast/addrspacecast, different intrinsic names)? Silent misrecognition is S0; loud rejection is fine.
3. jl_global#N literal certification (Bennett-hsm3, just landed): verify the claim "certified by live-session membership, never dereferenced", and try to construct a literal that slips through (const Ref, const tuple/struct, non-empty Memory literal, nested).
4. `callees.jl`/`callgraph.jl`/`julia_set.jl`: callee registry keyed how? Name collisions, specialization mismatches (same name, different arg types), recursion, `_LLVMRef` lifetimes / use-after-dispose of LLVM contexts and modules (LLVM.jl C API resource management is a classic source of segfaults — check every `dispose`/`context!` pairing).
5. `target_pin.jl`: does pinning actually make output host-independent (datalayout, triple, cpu features, fast-math flags)? What happens on non-x86 hosts?
6. Error paths (`errors.jl`): are unsupported constructs rejected loudly with context, or does anything fall through?
Known context: `worklog/108_*.md`, `docs/design/rearch-2026-08/` (report c1 covers extraction — verify or refute its claims, do not repeat them).
