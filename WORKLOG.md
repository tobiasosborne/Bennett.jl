...
>
> **Read [`worklog/052_2026-05-01_i2ca_strategy_dispatch.md`](worklog/052_2026-05-01_i2ca_strategy_dispatch.md) FIRST** — 2026-05-01 late evening, top entry is **Bennett-1pb close** — direct `llvm.sqrt.f64` / `llvm.exp.f64` / `llvm.exp2.f64` dispatch in `_handle_intrinsic` (3 new branches in `src/extract/instructions.jl`, ~30 LOC), wired to the existing `soft_fsqrt` / `soft_exp` / `soft_exp2` callees. 44-test contract file `test_1pb_llvm_transcendentals.jl` plus three `.ll` fixtures. f32/f16 forms fail loud per CLAUDE.md §13. **First opcode-parity bead picked up after the catalogue closed** — phase has shifted from code-review-catalogue to LLVM-IR-parity-with-Enzyme. Below it: **Bennett-kv7b / U65 close** (test-coverage epic; 16 sub-items total) and **Bennett-i2ca / U55 close** (`bennett(lr; strategy=...)` dispatch via `abstract type BennettStrategy` + 6 subtypes; 5 legacy aliases as zero-overhead forwarders).
>
