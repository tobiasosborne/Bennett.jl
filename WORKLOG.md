...
>
> **Read [`worklog/057_2026-05-04_s1zl_soft_tan.md`](worklog/057_2026-05-04_s1zl_soft_tan.md) FIRST** — 2026-05-04, **Bennett-s1zl close** (Tier C1.1 — first close in trig completion). `soft_tan` = faithful musl `__tan` port reusing `_rp_rem_pio2` from `fsin.jl`; `llvm.tan.f64` direct dispatch in `src/extract/instructions.jl` next to llvm.sin/cos. **Max ULP = 1 on 500k random samples (5 magnitude buckets up to 1e22), zero fails > 2 ULP.** Five gotchas captured incl. the in-kernel "big" branch (|y| ≥ 0.6744 fold to π/4 - |y|), the SET_LOW_WORD odd-arm precision trick, and `_RP_PIO4_BITS` reuse pattern. _CALLEES_FP_TRANS extended 14 → 15. `Bennett-Enzyme-Parity-NorthStar.md` Tier C1 updated. 10 C1 siblings remain — file order pinned in chunk 057's "Next agent starts here" (atan/asin/acos → atan2 → tanh → sinh/cosh → asinh/acosh/atanh).
>
...
|---|---:|---|
| [057_2026-05-04_s1zl_soft_tan.md](worklog/057_2026-05-04_s1zl_soft_tan.md) | ~120 | Session log — 2026-05-04 — **Bennett-s1zl close** (Tier C1.1 — first close in trig completion). `soft_tan` musl `__tan` port reusing `_rp_rem_pio2` from fsin.jl. 500k random samples, max ULP = 1, 0 fails > 2 ULP. Five gotchas captured incl. in-kernel big-arm fold and SET_LOW_WORD odd-arm precision trick. _CALLEES_FP_TRANS extended 14 → 15. |
