...
>
> **Read [`worklog/053_2026-05-01_dnh_research_nj6c.md`](worklog/053_2026-05-01_dnh_research_nj6c.md) FIRST** — 2026-05-01 late evening, two entries: **Bennett-nj6c close** (Bennett-dnh phase 1a — extends `_MUX_SHAPES_NW` from the original 6 hand-picked shapes to all (N,W) with N·W ≤ 64; 5 new shapes (3,8)/(5,8)/(6,8)/(7,8)/(3,16) close the `:unsupported` arm at `memory.jl:84-95`; Pkg.test 487,074 pass) AND **Bennett-dnh research/re-scope** (six parallel research agents, three codebase + three literature; persistent trees are dead per worklog/026 + literature, swap-mutate-swap is the architectural answer, QROAM is the cost win; **dnh re-scoped from RESEARCH to engineering** — 4 sub-beads filed: nj6c [closed], cb9y [open, multi-origin × runtime idx], 8guh [open P2, IRSwap 3+1], 6c6f [open, QROAM]). dnh closes when nj6c + cb9y both ship. Below it in chunk 052: **Bennett-1pb close** (direct `llvm.sqrt.f64` / `llvm.exp.f64` / `llvm.exp2.f64` dispatch), **Bennett-kv7b / U65 close** (test-coverage epic), **Bennett-i2ca / U55 close** (`bennett(lr; strategy=...)`).
>
...
|---|---:|---|
| [053_2026-05-01_dnh_research_nj6c.md](worklog/053_2026-05-01_dnh_research_nj6c.md) | ~155 | Session log — 2026-05-01 (late evening) — **Bennett-nj6c close** (dnh phase 1a — `_MUX_SHAPES_NW` extended to all (N,W) with N·W ≤ 64; 5 new shapes close the `:unsupported` arm) + **Bennett-dnh research/re-scope** (six-agent research sweep; persistent trees dead, swap-mutate-swap canonical, QROAM cost win; dnh re-scoped from RESEARCH to engineering with 4 sub-beads filed). |
