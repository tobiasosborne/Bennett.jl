...
>
> **Read [`worklog/054_2026-05-02_soft_log.md`](worklog/054_2026-05-02_soft_log.md) FIRST** — 2026-05-02, **Bennett-582 close** (`soft_log` / `soft_log2` / `soft_log10` + `llvm.log` / `llvm.log2` / `llvm.log10` dispatch). Branchless `src/softfloat/flog.jl` (~310 LOC) port of Arm Optimized Routines / musl `log.c` with main-path degree-6 + near-1.0-path degree-12 polynomials, Dekker hi/lo split, full §13 subnormal-INPUT testset. **100% bit-exact vs `Base.log` on 100k random samples**; soft_log2/log10 via change-of-base identity (≤1 ULP empirically). LLVM dispatch follows the Bennett-1pb / Bennett-h6f template; `.ll` fixtures + per-bead regression test ship alongside. Phase 1 of the Bennett-582 (log) + Bennett-emv (pow) workstream — pow needs log first. Phases 3-4 (soft_pow + llvm.pow/powi) next, will close Bennett-emv.
>
...
|---|---:|---|
| [054_2026-05-02_soft_log.md](worklog/054_2026-05-02_soft_log.md) | ~140 | Session log — 2026-05-02 — **Bennett-582 close**: `soft_log` / `soft_log2` / `soft_log10` + `llvm.log` / `llvm.log2` / `llvm.log10` dispatch. Arm port, 100% bit-exact `soft_log` vs `Base.log`. Phase 1+2 of log+pow workstream. |
