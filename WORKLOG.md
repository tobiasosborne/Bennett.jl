...
>
> **Read [`worklog/060_2026-05-06_m2bv_soft_tanh.md`](worklog/060_2026-05-06_m2bv_soft_tanh.md) FIRST** — 2026-05-06, **Bennett-m2bv close** (Tier C1.6 — `soft_tanh` + LLVM-ingest dispatch for BOTH `llvm.tanh.f64` intrinsic AND libm `@tanh` external call). **First hyperbolic primitive** in Tier C1; 6 of 11 done. Branchless port of Julia stdlib `Base.tanh(::Float64)` (julia 1.12 base/special/hyperbolic.jl:128-159) — three regimes: degree-10 polynomial in `x²` for `|x| ≤ 0.5` (coefficients verbatim from `Base.tanh_kernel`), `1 - 2/(exp(2|x|)+1)` for medium, `±1` saturation for `|x| ≥ 22`. ONE `soft_exp_fast` call total (FTZ branch unreachable since exp arg is non-negative — saves ~1.4M gates). ≤2 ULP vs `Base.tanh` on 100k random × 3 seeds × 4 magnitude buckets; **§13 subnormal-input bit-exactness implicit via the polynomial branch** (`x²` underflows → 0, `tanh_kernel(0) = 1`, `x · 1 ≡ x`) — 0 ULP across all 1074 subnormal binades × both signs. `_CALLEES_FP_TRANS` extended 19 → 20. **`sinh` is the next pickup** (Tier C1.7 — note `Base.sinh` uses TWO exp calls in the medium regime; consider whether the symmetry `(e^{2|x|}-1)/(2·e^{|x|})` lets us fold to one exp at lower gate cost).
>
