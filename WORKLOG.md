...
>
> **Read [`worklog/061_2026-05-06_ky5n_soft_sinh.md`](worklog/061_2026-05-06_ky5n_soft_sinh.md) FIRST** — 2026-05-06, top entry: **Bennett-bybh close** (Tier C1.8 — `soft_cosh` + LLVM-ingest dispatch for both `llvm.cosh.f64` intrinsic and libm `@cosh`). **Third hyperbolic primitive**; 8 of 11 done. Three-regime branchless port mirroring ky5n with three localised differences (cosh is even, medium is sum `(E+1/E)/2` so zero cancellation, polynomial has no `x` factor). Polynomial coefficients verbatim from `Base.cosh_kernel`. ONE `soft_exp_fast` call via regime-selected arg. ≤2 ULP across 100k random × 3 seeds × 5 buckets; **§13 contract DIFFERENT for cosh: `cosh(any subnormal) === 1.0` bit-exactly** (since `1 + subnormal² = 1.0` in fp64) across all 1074 binades × ±. **3+1 protocol skipped** per §2 exception (mechanical extension of ky5n; documented in implementation header). _CALLEES_FP_TRANS extended 21 → 22. Below it: **Bennett-ky5n close** (Tier C1.7 — `soft_sinh` + LLVM-ingest dispatch for BOTH `llvm.sinh.f64` intrinsic AND libm `@sinh` external call). **Second hyperbolic primitive** in Tier C1. Three-regime branchless port: degree-8 polynomial in `z=x²` for `|x| ≤ 1.0` (Julia stdlib `Base.sinh_kernel` minimax coefficients), `(E - 1/E)/2` with `E = exp(|x|)` for `1 < |x| < 709`, `(0.5·E)·E` with `E = exp(|x|/2)` for `|x| ≥ 709`. **ONE `soft_exp_fast` call total** via regime-selected argument. 3+1 synthesis pivoted from Proposer B's unified `(E²-1/E²)/2` (which had 5 chained fmul ops → 3-4 ULP at |x|≈1.4) to A's regime-split `(E-1/E)/2` (3 ops after exp → ≤2 ULP). §13 subnormal-INPUT bit-exact (0 ULP across 1074 binades × ±). Drive-by finding: `soft_exp_fast` has a small NaN-producing bug for inputs in `(~709.78, ~709.79)`; worked around by setting Bennett-ky5n's huge-regime threshold conservatively at 709.0 (vs Julia's 709.7822265633563). CRITICAL load-bearing operator ordering: `(0.5·E)·E` not `(E·E)·0.5` for the huge arm. `_CALLEES_FP_TRANS` extended 20 → 21. **`cosh` is the next pickup** (Tier C1.8 — even function, `(E+1/E)/2`, can share huge-arm infrastructure with sinh).
>
