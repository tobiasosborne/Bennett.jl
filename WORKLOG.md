...
|---|---:|---|
| [059_2026-05-06_7goc_soft_atan2.md](worklog/059_2026-05-06_7goc_soft_atan2.md) | ~190 | Session log — 2026-05-06 — **Bennett-7goc close** (Tier C1.5 — `soft_atan2` + dispatch for both `llvm.atan2.f64` and libm `@atan2`; built on `soft_atan` per §12; ≤2 ULP on 100k random × 3 seeds × 4 quadrants; drive-by fix tightened all six trig prefixes with trailing `.` to prevent silent miscompile that matched `llvm.atan2.f64` against the `llvm.atan` arm pre-7goc). _CALLEES_FP_TRANS 18 → 19. |
